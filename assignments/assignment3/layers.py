...
class ConvolutionalLayer:
    ...
    def unpad(self, X_pad_like):
        padding = self.padding
        batch_size, in_height_pad, in_width_pad, in_channels = X_pad_like.shape
    ...
    def forward(self, X):
        ...
        X_pad = np.zeros((batch_size, in_height_pad, in_width_pad, in_channels))
        X_pad[:, padding:in_height + padding, padding:in_width + padding, :] = X
        self.X_pad = X_pad
        ...
        out_height = in_height_pad - filter_size + 1
        out_width = in_width_pad - filter_size + 1
        ...
        for y in range(out_height):
            for x in range(out_width):
                ...
                Z_target = np.matmul(X_window_2d, W_2d) + self.B.value          # batch @ out_channels
    ...
    def backward(self, d_out):
        ...
        for y in range(out_height):
            for x in range(out_width):
                ...
                d_result_window_2d = np.matmul(d_out_2d, W_2d.T)                            # batch @ (filter * filter * in_channels)
                dLdW_2d = np.matmul(X_window_2d.T, d_out_2d)                                # (filter * filter * in_channels) @ out_channels
        ...
        padding = self.padding
        in_height_unpad = in_height - 2 * padding
        in_width_unpad = in_width - 2 * padding

        return d_result[:, padding:in_height_unpad + padding, padding:in_width_unpad + padding, :]
